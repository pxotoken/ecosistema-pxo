...
export class PricingService {
  ...
  private mapPXOToMXN(pair: string): string {
    ...
    if (upperPair.includes('PXO')) {
      ...
      if (parts.length === 2) {
        ...
        if (base === 'PXO') return `MXN${quote}`;
        if (quote === 'PXO') return `${base}MXN`;
      }
    }
    return pair;
  }
  ...
  private async getPricingRule(pair: string): Promise<PricingRule> {
    ...
    if (data.spread_buy < 0 || data.spread_sell < 0) {
      throw new Error(`Invalid spread values for pair ${pair}. Spreads must be non-negative.`);
    }
